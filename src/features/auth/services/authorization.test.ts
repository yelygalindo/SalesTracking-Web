...
describe("authorization", () => {
  it("evalúa permisos sin distinguir mayúsculas y minúsculas", () => {
    ...
    expect(authorization.canAll(["dashboard.read", "customers.read"])).toBe(
      false,
    );
  });
  ...
  it("reconoce el permiso administrativo implícito de platform_admin", () => {
    const authorization = createAuthorization({
      ...user,
      roles: ["platform_admin"],
      permissions: [],
    });
    expect(authorization.can("companies.create")).toBe(true);
