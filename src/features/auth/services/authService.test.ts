...
describe('authService', () => {
  ...
  it('inicia sesión con el contrato exacto y guarda tokens', async () => {
    publicMock.onPost('/api/auth/login').reply(200, { user, accessToken: 'a', refreshToken: 'r', expiresAtUtc: '2027-01-01T00:00:00Z' })
    ...
    expect(tokenStore.get()?.refreshToken).toBe('r')
  })
  ...
  it('hace logout remoto y limpia localmente incluso si la API falla', async () => {
    tokenStore.replace({ accessToken: 'a', refreshToken: 'r', expiresAtUtc: '2027-01-01T00:00:00Z' }); apiMock.onPost('/api/auth/logout').networkError()
    await expect(authService.logout()).rejects.toBeDefined()
    expect(JSON.parse(apiMock.history.post[0].data)).toEqual({ refreshToken: 'r' }); expect(tokenStore.get()).toBeNull()
  })
})
