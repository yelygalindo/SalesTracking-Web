...
describe('customerService', () => {
  ...
  it('envía las filas y la selección de una importación validada', async () => {
    mock.onPost('/api/customers/imports/validate').reply(200, { importId: 'import-1', totalRows: 1, validRows: 1, warningRows: 0, invalidRows: 0, rows: [] })
    mock.onPost('/api/customers/imports/import-1/commit').reply(200, { created: 1, failed: 0, items: [] })
    await customerService.validateImport([{ rowNumber: 2, name: 'Ana' }])
    await customerService.commitImport('import-1', { includeAllValidRows: false, includeWarningRows: false, selectedRows: [2] })
    expect(JSON.parse(mock.history.post[0].data)).toEqual({ rows: [{ rowNumber: 2, name: 'Ana' }] })
