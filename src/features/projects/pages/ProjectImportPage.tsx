...
export function ProjectImportPage() {
  ...
  const template = useMutation({
    mutationFn: projectService.importTemplate,
    onSuccess: (blob) => downloadBlob(blob, "plantilla-proyectos.xlsx"),
    onError: (error: Error) => notify(error.message, "error"),
  });
  const validation = useMutation({
    mutationFn: projectService.validateImport,
    onSuccess: (value) => {
      setPreview(value);
      setSelectedRows(
        value.rows
  ...
  const commit = useMutation({
    mutationFn: () => {
      ...
      return projectService.commitImport(preview.importId, {
  ...
  const downloadErrors = () => {
    ...
    exportExcel(
      `proyectos-no-importados-${new Date().toISOString().slice(0, 10)}.csv`,
      [
        { label: "Fila", value: (item) => item.rowNumber },
        {
          label: "Proyecto",
          value: (item) => byNumber.get(item.rowNumber)?.data.name ?? "",
        },
        {
          label: "Cliente",
          value: (item) =>
            byNumber.get(item.rowNumber)?.data.customerExternalId ?? "",
        },
  ...
  return (
    <main className="customers-content customer-import-page">
      <PageHeader
        ...
        description="Carga proyectos de forma masiva con validación previa por fila."
        action={
      ...
      {step === 0 ? (
        <section className="import-start-grid">
          <article className="import-action-card">
            ...
            <p>
              Descarga el archivo Excel con las columnas admitidas y ejemplos
              para completar.
            </p>
            <button
          ...
          <article className="import-action-card featured">
            ...
            <p>
              Sube la plantilla, revisa las validaciones y selecciona qué
              registros importar.
            </p>
          ...
          {step === 1 && (
            <section className="import-wizard-card">
              ...
              <FileDropZone
                ...
                onDrop={(nextFile) => void loadFile(nextFile)}
                onClear={() => {
                  setFile(null);
                  setRows([]);
                }}
              ...
              <footer className="import-wizard-actions">
                ...
                <button
                  ...
                  disabled={!rows.length || validation.isPending}
                  onClick={() => validation.mutate(rows)}
                >
          ...
          {step === 2 && preview && (
            <section className="import-wizard-card import-review-card">
              <header>
                <h2>Revisar datos</h2>
                <p>
                  Selecciona los registros que deseas importar. Las filas con
                  errores bloqueantes no pueden seleccionarse.
                </p>
              </header>
              <div className="import-review-toolbar">
                <div className="import-status-filters">
                  {(
                    ["all", "valid", "warning", "invalid"] as StatusFilter[]
                  ).map((status) => (
                    <button
                    ...
                    >
                      {statusLabel(status)}{" "}
                      {status !== "all" && `(${statusCount(preview, status)})`}
                    </button>
                  ))}
                </div>
              ...
              <footer className="import-wizard-actions">
                ...
                <button
                  className="action-primary"
                  disabled={!selectedRows.length || commit.isPending}
                  onClick={() => commit.mutate()}
                >
                  {commit.isPending
                    ? "Importando…"
                    : `Importar ${selectedRows.length} proyectos`}
                </button>
              </footer>
            </section>
          )}
            ...
            <section className="import-wizard-card import-result-card">
              ...
              <div className="import-result-metrics">
                <article>
                  ...
                  <strong>{result.items?.length ?? selectedRows.length}</strong>
                </article>
                ...
                <article className="danger">
                  ...
                  <strong>{result.failed ?? failedItems.length}</strong>
                </article>
              </div>
              {failedItems.length > 0 && (
                <div className="import-failed-records">
                  <header>
                    <div>
                      <h3>Registros no importados</h3>
                      <p>
                        Corrige estos registros antes de volver a intentarlo.
                      </p>
                    </div>
              ...
              <footer className="import-wizard-actions">
                <Link to="/projects">Ir al listado</Link>
...
function WizardSteps({ current }: { current: number }) {
  return (
    <ol className="import-wizard-steps">
      {["Cargar archivo", "Revisar datos", "Resultado"].map((label, index) => {
        const number = index + 1;
        return (
          <li
            className={
              current === number ? "active" : current > number ? "complete" : ""
            }
            key={label}
          >
            <span>{number}</span>
      ...
      {file ? (
        <>
          ...
          <div>
            ...
            <span>{count.toLocaleString("es-BO")} registros detectados</span>
          </div>
          <button aria-label="Quitar archivo" onClick={onClear}>
            <XCircle />
          </button>
        </>
      ) : (
        <>
          ...
          <button onClick={onSelect}>Seleccionar archivo</button>
          <small>Formato permitido: XLSX</small>
        </>
      )}
    </div>
  );
      ...
      <table>
        ...
        <tbody>
          {rows.map((row) => {
            ...
            return (
              <tr className={`row-${row.status}`} key={row.rowNumber}>
                <td>
                  <input
                    type="checkbox"
                ...
                <td>
                  {row.data.progressPercentage == null
                    ? "—"
                    : `${String(row.data.progressPercentage)}%`}
                </td>
                <td>
                ...
                <td>
                  {issues.length ? (
                    <details>
                      <summary>
                        {issues.length}{" "}
                        {issues.length === 1 ? "detalle" : "detalles"}
                      </summary>
...
const displayDate = (value: unknown) => {
  ...
  return Number.isNaN(parsed.getTime())
    ? String(value)
    : new Intl.DateTimeFormat("es-BO").format(parsed);
};
const statusLabel = (status: StatusFilter) =>
  ({ all: "Todos", valid: "Válido", warning: "Advertencia", invalid: "Error" })[
    status
  ];
const statusCount = (
