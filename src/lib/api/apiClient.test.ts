...
describe('auth interceptor', () => {
  it('comparte un refresh concurrente y reintenta cada solicitud una vez', async () => {
    const store = new TokenStore(new MemoryStorage()); store.replace({ accessToken: 'expired', refreshToken: 'rotate-me', expiresAtUtc: '2026-01-01' })
    ...
    publicMock.onPost('/api/auth/refresh-token').reply(async () => { refreshCalls += 1; await new Promise(resolve => setTimeout(resolve, 10)); return [200, { accessToken: 'new-a', refreshToken: 'new-r', expiresAtUtc: '2027-01-01' }] })
    ...
    expect(results.map(result => result.data.ok)).toEqual([true, true]); expect(refreshCalls).toBe(1); expect(store.get()?.refreshToken).toBe('new-r'); expect(onFailure).not.toHaveBeenCalled()
  })
  it('limpia la sesión si el refresh falla y no entra en bucle', async () => {
    const store = new TokenStore(new MemoryStorage()); store.replace({ accessToken: 'expired', refreshToken: 'invalid', expiresAtUtc: '2026-01-01' })
    const onFailure = vi.fn(); const { apiClient, publicClient } = createApiClients(store, onFailure)
    const apiMock = new MockAdapter(apiClient); const publicMock = new MockAdapter(publicClient)
    apiMock.onGet('/protected').reply(401); publicMock.onPost('/api/auth/refresh-token').reply(401, { message: 'Refresh token inválido o expirado.' })
